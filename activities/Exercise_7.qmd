---
title: "Lesson 7 - Wrangling + Dates"
...
*You might delete information that you need.*
\
\
\
\
...
# One gives the number of the month, the other gives the name of the month
month(today)
...
# It's the number of the week in the year
week(today)
```

```{r}
# What do these lines produce / what's their difference?
# The first is the day in the month, the second is the day in the year.
mday(today)
...
```{r}
# What do these lines produce / what's their difference?
# One gives the number of the day of the week, the other gives the name.
wday(today)
...
# Today is after the date 2/14/24, not before. 
...
Birthdays |>
  group_by(state) |>
  summarize(n_distinct(Birthdays))
...
Birthdays |>
  group_by(state) |>
  mutate(total_birth_state = sum(births)) |>
  select(state, total_birth_state)
...
ggplot(monthly_births, aes(x = month_year, y = total_births, group = Year, color = as.factor(Year))) +
  geom_point() +
  geom_line()
```
