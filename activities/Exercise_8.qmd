...
```{r}
# Units of observation = inidvidual penguins
head(penguins)

# Units of observation = penguin species by sex
head(penguin_avg)
```

2.  Did the reshaping process result in any information loss from the original data?

*Yes because we lost data on individual observations in exchange for general species data.*
\
\
\
\
...
*Units of obersvation are the three species*
...
*Because you are renaming the columns in pivot_longer so they need to be in quotations*
...
```{r}
more_food |>
  pivot_longer(cols = c(Shish, FrenchMeadow, DunnBros), names_to = "Restaurant", values_to = "Orders")
```
...
*The way the days are expressed*

-   How did this impact how the values are recorded in the `day` column?

*It removed the day_ prior to each value*
...
*Overall, the longer subjects without sufficent sleep, the slower their reaction time became.*

-   Is this the same for everybody? What differs?

*This is not the same for everyone, subject 335, 332, and 310 experienced drops in their reaction times later in the week.*
...
*They don't specify that they are days, they are currently just numbers*
...
ggplot(billboard_kryptonite, aes(x = wk1, y = wk2)) +
  geom_point() +
...
```{r}
billboard |>
  filter(artist %in% c("Counting Crows", "Hill, Faith")) |>
  filter(track %in% c("Hanginaround", "Breathe"))


  #ggplot(aes(nov_1999), x = )
```
