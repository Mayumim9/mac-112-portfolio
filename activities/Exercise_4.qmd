...
# This differs from our histogram of repub_pct_20 because there are now two variables being measured. 
...
# What happens if you change the shape to another number? (the shape of the individual points change)
ggplot(elections, aes(y = repub_pct_20, x = repub_pct_16)) +
  geom_point(shape = 3)
```
...
```{r eval = FALSE}
# Two bad things about this plot are that the labels are unclear and teh colors don't align with their labels. 
ggplot(elections, aes(x = repub_pct_20, fill = historical)) +
  geom_density()
```

```{r eval = FALSE}
# What does scale_fill_manual do? (this function assigns specific colors)
ggplot(elections, aes(x = repub_pct_20, fill = historical)) +
  geom_density() +
...
```{r eval = FALSE}
# What does alpha = 0.5 do? (it changes the transparency of the colors)
...
```{r eval = FALSE}
# What does facet_wrap do?! (It seperates out each categorical variable into its own plot)
ggplot(elections, aes(x = repub_pct_20, fill = historical)) +
  geom_density() +
  scale_fill_manual(values = c("blue", "purple", "red")) +
