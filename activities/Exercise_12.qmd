...
library(tidyverse)
library(readr)
imdb_messy <- read_csv("/Users/mayumimorgan/Downloads/Comp112-03/mac-112-portfolio/data/imdb_5000_messy.csv")
View(imdb_messy)
```
...
*It arranged individual observations in alphabetical or numerical order.*
...
**It provided the average,minimum, and maximum observations for each quantitative variable.**

-   What type of info is provided on categorical variables?

**It privdes the number of observations per categorical variable.**

-   What stands out to you in these summaries? Is there anything you'd need to clean before using this data?

**I would clean out the NA's, and the additional measurements for qualitative variables that I won't use.**
...
*You have the same variable but they are named different things. For example, there is a "black and white" variable and a "B&W" variable. They need to be combined into a single variable.*
...
*Color is the following three categories: COLOR, Color, and color. Combined, there are 4,815 movies in the color category, 30 in COLOR, 30 in color, and 4,755 in Color.*
...
*If we use R code, we can store everything in one place and can go back if we need to.*
